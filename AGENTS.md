...
本项目存在三种编辑器渲染通道，互不共享 DOM 和事件机制：

| 模式 | 技术实现 | 编辑区域 DOM |
|------|----------|-------------|
| 写作模式（wysiwyg） | Milkdown Crepe / ProseMirror | `.typora-editor > .milkdown .ProseMirror` |
| 源码模式（source） | 原生 `<textarea>` | `.editor-textarea` |
| 分屏模式（split） | textarea + 预览 | `.editor-split__pane` 内 textarea + `.markdown-preview` |

**核心认知**：新增写作模式不是普通 UI 改动，而是新增一条完整的编辑器运行通道。CSS 继承、事件冒泡、Markdown 序列化路径在三模式下均不同。

### 10.1 三模式回归矩阵

以下改动类型必须做**三模式手动回归**：

| 改动类型 | 源码模式 | 写作模式 | 分屏模式 | 关键风险 |
|----------|:---:|:---:|:---:|----------|
| 字体/字号/主题 CSS | 必验 | 必验 | 必验 | Crepe CSS 可覆盖继承样式 |
| 图片插入（拖拽/粘贴/按钮） | 必验 | 必验 | 必验 | 写作模式通过 ProseMirror transaction；源码模式操作 textarea value |
| 查找替换 | 必验 | 必验 | 必验 | 写作模式用 ProseMirror Decoration；源码用 textarea selection |
| 快捷键（Ctrl+B/I/S/O/N） | 必验 | 必验 | 不适用 | 写作模式内快捷键可能被 Crepe 拦截 |
| 保存/另存为/导出 | 必验 | 必验 | 必验 | 三模式必须输出相同 Markdown |
| 大纲跳转 | 必验 | 必验 | 不适用 | 写作模式用 `scrollIntoView`；源码用 line offset |
| 编辑器容器/DOM 结构 | 必验 | 必验 | 必验 | 三模式有独立容器和 className |

### 10.2 写作模式专属禁止事项

1. 不允许直接 `document.querySelector` 修改 `.ProseMirror` 内部 DOM。
2. 不允许绕过 ProseMirror transaction 插入/删除节点。
3. 不允许维护一份与 ProseMirror state 独立的 Markdown 字符串。
4. 不允许在未检查 `generationRef` 生命周期的情况下操作 Crepe 实例。
5. 不允许在写入模式下使用 textarea 的 `setSelectionRange` / `scrollTop` 逻辑。

### 10.3 写作模式 CSS 覆盖规范

任何需要覆盖 Milkdown Crepe 默认样式的 CSS 规则，必须满足：

1. 选择器前缀使用 `.typora-editor .milkdown .ProseMirror`（特异性 0-4-0，高于 Crepe 默认的 0-3-0）。
2. 优先使用 CSS 自定义属性（`--crepe-font-*`、`--editor-font-*`）传递值，避免 `!important`。
3. 对 `font-family` / `font-size` 的修改必须同时覆盖 `p` / `li` / `blockquote` / `td` / `th` / `h1-h6` 等 Crepe 已硬编码的元素。
4. 不遗漏代码块 `code` / `pre` 内的字体回退链。
...
```text
[ ] 源码模式：功能正常，无报错
[ ] 写作模式：功能正常，ProseMirror state 未损坏
[ ] 分屏模式：源码侧和预览侧均正常
[ ] 三模式间切换：状态保持一致，光标/选区不丢失
[ ] Markdown 输出：三模式下序列化结果一致
[ ] 关闭重开：文档内容与关闭前一致
```
